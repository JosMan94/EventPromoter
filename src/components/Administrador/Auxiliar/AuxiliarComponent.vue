<template>
  <!-- Buscador -->
  <article class="mb-6">
    <h2 class="font-bold text-xl xl:text-3xl mb-5 xl:mb-10 text-text-blue">Auxiliares</h2>
    <form class="grid xl:grid-cols-2 gap-6">
      ...
      <div class="flex justify-between">
        ...
        <router-link
          :to="{ name: 'Administrador', params: { viewAdmin: 'create-auxiliar' } }"
  ...
  <article>
    <div class="rounded-xl overflow-hidden shadow-lg mb-10">
      <header class="hidden xl:grid grid-cols-14 gap-5 table-head">
        ...
        <!-- <p class="col-span-2 flex items-center gap-4">REGALO</p> -->
        <!-- <p class="col-span-2 flex items-center gap-4">
          ACCIONES
          <img src="../../../assets/images/arrow-down.png" alt="" />
        </p> -->
      </header>
      <span v-for="data in auxiliares" :key="data">
        <div class="grid grid-cols-2 xl:grid-cols-14 gap-5 table-row">
          ...
          <p class="xl:col-span-2 xl:flex items-center gap-4">
            ...
            <router-link
              :to="{ name: 'Administrador', params: { viewAdmin: 'edit-auxiliar' } }"
          ...
          <!-- <p class="xl:col-span-2 xl:flex items-center gap-4">
            <span class="block xl:hidden text-text-blue mb-6">Regalo:</span>
            <router-link
              :to="{ name: 'Administrador', params: { viewAdmin: 'regalo-invitado' } }"
              @click.prevent="sendDataEvent(data)"
              class="rounded-md px-6 py-4 bg-red-700 text-white ring-2 ring-red-700 hover:bg-white hover:text-red-700 transition-colors"
            >
              Regalo
            </router-link>
          </p> -->
    ...
    >
      ...
      <span v-for="data in pagination.links" :key="data">
        <button
          @click.prevent="getAuxiliar(true, data.label)"
    ...
    <div class="mt-12 flex xl:hidden justify-end items-center gap-12">
      ...
      <div class="flex items-center gap-4">
        <figure
          ...
          @click.prevent="getAuxiliar(true, pagePast)"
        ...
        <figure
          ...
          @click.prevent="getAuxiliar(true, pageNext)"
          class="cursor-pointer p-3"
        >
          <img src="../../../assets/images/arrow-right.png" alt="Next" />
        </figure>
...
export default {
  ...
  methods: {
    sendDataEvent(data) {
      this.$emit("dataAuxiliar", data);
    },
    async search() {
      var objPage = new Object();
      objPage.clave = this.clave;
      objPage.table = "adminAuxiliar";
    ...
    async getAuxiliar(option, data) {
      ...
      objPage.filterBy = "auxiliar";
      var result = await promotorService.getPromotor(objPage);
      if (result.success) {
        this.$store.state.alert.status = true;
        this.$store.state.alert.type = "success";
        this.$store.state.alert.text = "Listado de auxiliares";

        this.pagination.state = false;
        this.auxiliares = result.data.data;
        ...
        this.$store.state.alert.status = true;
        this.$store.state.alert.type = "error";
        this.$store.state.alert.text = "Error al mostrar los auxiliares";
      }
    },
  },
  watch: {
