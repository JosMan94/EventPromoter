<template>
  ...
  <article class="mb-6">
    ...
    <form class="grid xl:grid-cols-2 gap-6">
      ...
      <div class="flex justify-between">
        ...
        <router-link
          :to="{ name: 'Administrador', params: { viewAdmin: 'create-admin' } }"
  ...
  <article>
    <div class="rounded-xl overflow-hidden shadow-lg mb-10">
      ...
      <span v-for="data in admin" :key="data">
        <div class="grid grid-cols-2 xl:grid-cols-14 gap-5 table-row">
          ...
          <p class="xl:col-span-2 xl:flex items-center gap-4">
            ...
            <router-link
              :to="{ name: 'Administrador', params: { viewAdmin: 'edit-admin' } }"
    ...
    >
      ...
      <span v-for="data in pagination.links" :key="data">
        <button
          @click.prevent="getAdmin(true, data.label)"
    ...
    <div class="mt-12 flex xl:hidden justify-end items-center gap-12">
      ...
      <div class="flex items-center gap-4">
        <figure
          ...
          @click.prevent="getAdmin(true, pagePast)"
        ...
        <figure
          ...
          @click.prevent="getAdmin(true, pageNext)"
          class="cursor-pointer p-3"
        >
          <img src="../../../assets/images/arrow-right.png" alt="Next" />
        </figure>
...
export default {
  ...
  methods: {
    sendDataEvent(data) {
      this.$emit("dataAdmin", data);
    },
    async search() {
      var objPage = new Object();
      objPage.clave = this.clave;
      objPage.table = "adminAdmin";
    ...
    async getAdmin(option, data) {
      ...
      objPage.filterBy = "admin";
      var result = await promotorService.getPromotor(objPage);
      if (result.success) {
        this.$store.state.alert.status = true;
        this.$store.state.alert.type = "success";
        this.$store.state.alert.text = "Listado de administradores";
        ...
        this.$store.state.alert.status = true;
        this.$store.state.alert.type = "error";
        this.$store.state.alert.text = "Error al mostrar los administradores";
      }
    },
  },
  watch: {
